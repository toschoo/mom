...
where
  ...
  mapOneOrMany Many f ss = mapM f ss
  ...
  sendQAll q | null (qSubs q) = return q
             | otherwise      = do
                 sbs  <- sendAllInQ q (qSubs q)
                 sbs' <- catMaybes <$> mapOneOrMany (qPat q) sendSubAll sbs
                 return q {qSubs = sbs'}

  sendAllInQ :: Queue -> [Sub] -> IO [Sub]
  sendAllInQ q sbs = do
    mbF <- pop (qMsgs q)
    case mbF of
      Nothing -> return sbs
  ...
  sendSub f s = do 
    let msg       = "msg-1" -- <- getMsgId
    let (_, sid)  = parseSubId (subId s)
    case F.sndToMsg msg (show sid) f of
      Nothing -> return $ Just s -- protocol error
      Just f' -> do
        w <- Fifo.empty (subMsgs s)
        if w 
          then send f' s
          else push (subMsgs s) f' >> (return $ Just s)
               -- add to database
  ...
  send f s = catch (do S.send (conWr   $ subCon s) 
                              (conSock $ subCon s) f
                       -- remove from database 
                       return $ Just s)
                   (\e -> do reportSilent NOTICE $ 
