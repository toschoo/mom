...
where
  ...
  type SrvNode = (ServiceName, Service)
  type SrvTree = Map ServiceName Service
  ...
  insert :: Identity -> ServiceName -> IO ()
  insert i s = do
    mbW <- lookupW i
    case mbW of
      Just _  -> return ()
  ...
  getServiceName :: Identity -> IO (Maybe ServiceName)
  getServiceName i = do
    mbW <- lookupW i
    case mbW of
  ...
  getWorker :: ServiceName -> IO (Maybe Identity)
  getWorker s = do
    mbS <- lookupS s
    case mbS of
      Nothing -> return Nothing
  ...
  statPerService :: ServiceName -> IO (ServiceName, Int, Int)
  statPerService s = do
    mbS <- lookupS s
    case mbS of
  ...
  lookupS :: ServiceName -> IO (Maybe Service)
