...
where
  ...
  str2Msg :: MonadResource m => C.Conduit String m Msg
  str2Msg = C.awaitForever $ \x -> C.yield $ Msg x

  outC :: (Show a, MonadResource m) => C.Conduit (Message a) m a
  outC = C.awaitForever $ \i -> let o = msgContent i
                                 in liftIO (print o) >> C.yield o

  dropC :: MonadResource m => C.Consumer i m ()
  ...
  handshake qn = do 
    withConnection "localhost" 61613 [] [] $ \c -> do
      ...
      runResourceT $ sourceList req $=
                     str2Msg        $$
                     qMultiSink outQ nullType [] 
      runResourceT $ qMultiSource inQ (-1) $=
                     outC $$ dropC
